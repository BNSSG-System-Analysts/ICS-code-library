...
##Plots 2 reference lines on the chart and puts the value lables on the sides like columns

# Calculate the maximum lsoa_popperc across all age bands to find the position reference point for the labels
max_lsoa_popperc_all <- max(dat_lsoa$lsoa_popperc)
...
pop_plot_BNSSG <- ggplot(dat_lsoa)+
  ...
  geom_line(data = dat_BNSSG %>% filter(sex == "Female"), 
            mapping = aes(x=ageband,y=BNSSG_popperc), color = "black",group = 1, linewidth = 1.2)+
  #below is the male line
  geom_line(data = dat_BNSSG %>% filter(sex == "Male"), 
            mapping = aes(x=ageband,y=BNSSG_popperc), color = "black",group = 1, linewidth = 1.2)+
  
  geom_line(data = dat_SB %>% filter(sex == "Female"), 
            mapping = aes(x=ageband,y=SB_popperc), color = "#005EB8",group = 1, linewidth = 1.2)+
  #below is the male line
  geom_line(data = dat_SB %>% filter(sex == "Male"), 
            mapping = aes(x=ageband,y=SB_popperc), color = "#005EB8",group = 1, linewidth = 1.2)+
  #geom_text(aes(x = ageband, y = lsoa_popperc + signal * 2.2, #use to add labels to bars
  #              label = paste0(round(abs(lsoa_popperc), 1), "%")),
  #          size = 3) +    # Use size to adjust the text size    
  
  geom_text(aes(x = ageband, 
                y = ifelse(sex == "Female", -max_lsoa_popperc_all + signal * 1.5, max_lsoa_popperc_all + signal * 1.5),
               label = paste0(round(abs(lsoa_popperc), 1), "%")),
            size = 3) +    # Use size to adjust the text size
  
  
  coord_flip()+
  scale_fill_manual(name='',values=c("#009639","#B3B3B3"))+
  scale_color_manual(name='',values=c("#009639","#B3B3B3"))+
  scale_y_continuous(breaks=seq(-50,50,1),
                     labels=function(x){abs(x)})+
  labs(x='',y='Population (%)',
       title='Bedmister West (Bristol 039F) LSOA population compared to \nBNSSG and and South Bristol locality registered populations',
       subtitle=paste('Registered population as at the 1st July 2024,\nBlack line shows % BNSSG population \nBlue line shows % South Bristol locality population'), #\n inserts a text wrap
       caption=' ')+
  cowplot::theme_cowplot()+
  bnssgtheme()+
  #Changes the size of the text on the axis to be different for the standard set in BNSSGtheme()
  theme(plot.title = element_text(size = 11),
        #axis.text.x = element_text(size = 8), #element_blank(), # Adjust the size value as needed
        axis.text.x = element_blank(),
        axis.text.y = element_text(size = 8),  # Adjust the size value as needed
        legend.text = element_text(size = 8),
        )  

# Save the plot as a PNG file
ggsave("C:/Annes offline R/community connectors/02 Data_Images/population plot_per all labels at sides.png", plot = pop_plot_BNSSG, width = 7, height = 6, dpi = 300)
